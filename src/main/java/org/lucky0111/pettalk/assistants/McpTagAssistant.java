...
public interface McpTagAssistant {
    @SystemMessage("""
            ...
            - 태그는 한글로 작성된 간결한 키워드여야 합니다
            - 문장 형태가 아닌 핵심 키워드만 추출하세요
            - DB에 이미 존재하는 태그는 반드시 DB 태그 목록에서 선택하세요
            - 동일한 개념을 다른 단어로 표현하지 말고, DB에 있는 태그 문자열 그대로 사용하세요
            - **DB에 없는 태그 개념은 새로운 태그를 간결하게 생성하세요**
            ...
            **올바른 응답**: 소형견,배변
            ...
            ### 활용 가능한 툴
            - getTagsWithDB: DB에서 가져온 모든 태그 목록을 반환합니다. 이 목록에서 적절한 태그를 선택하여 사용하세요.
            """)
